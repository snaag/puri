...
const textControll = require('./textControll')
...
async function URLToDB(request){
    ...
    try{
        const noteInfo = await Notes.create({
            user_userId,
            picUrl : "https://purireviewnote.s3.ap-northeast-2.amazonaws.com"+"/"+key,
            resultText,
            comment,
            review
        })
      //  response.status(200).send("db")
      return { id: noteInfo.dataValues.id, user_userId: noteInfo.dataValues.user_userId}
    }catch(e){
        console.log(e)
    }
}
...
module.exports = {
    ...
    SendToMathpix: async (request, response) => {
        ...
        let data ={
            ...
            "src":"https://purireviewnote.s3.ap-northeast-2.amazonaws.com"+"/"+key,
